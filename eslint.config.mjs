...
const rule = {
  meta: {
    type: "maintenance",
    docs: {
      description: "Ensure a specific comment exists at the top of the document",
      category: "Mandatory",
      recommended: true
    },
    fixable: "code"
  ...
  create(context) {
    ...
    return {
      Program(node) {
        ...
        if (!(comments.length && comments[0].range[0] === 0 && comments[0].value == commentText)) {
          context.report({
            node: comments[0] ?? node.body[0],
            message: "Copyright comment not found at the top",
            fix(fixer) {
              return fixer.insertTextBefore(comments[0] ?? node.body[0], `/*${commentText}*/\n\n`);
            }
          });
          return;
        }
