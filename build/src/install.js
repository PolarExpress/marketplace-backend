 ...
 */
const installRoute = (ctx) => (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    res.json(yield ctx.prisma.user.update({
        where: { id: user.id },
        data: {
            installedAddons: {
                connect: { id: addon.id }
 ...
 */
const uninstallRoute = (ctx) => (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { userId, addonId } = req.body;
    // Find the user by id. If the user is not found, throw an error.
    const user = yield ctx.prisma.user.findUnique({
        where: { id: userId },
